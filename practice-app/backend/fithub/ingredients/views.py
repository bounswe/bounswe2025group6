...
class WikidataViewSet(viewsets.ViewSet):
    ...
    swagger_auto_schema(
        operation_description="Get allergen info of an ingredient/meal.",
        manual_parameters=[
            openapi.Parameter('name', openapi.IN_QUERY, description="Ingredient or meal name", type=openapi.TYPE_STRING, required=True)
        ],
        responses={200: openapi.Response('Allergen info')},
        tags=["IngredientWikidata"]
    )

    @action(detail=False, methods=['get'], url_path='allergens')
    def allergens(self, request):
        name = request.query_params.get("name")
        if not name:
            return Response({'error': 'Query parameter "name" is required.'}, status=status.HTTP_400_BAD_REQUEST)

        # Wikidata doesn't always store allergens as structured data, but this is an example
        qid = self.get_wikidata_entity(name)
        if not qid:
            return Response({'error': 'Item not found in Wikidata.'}, status=status.HTTP_404_NOT_FOUND)

        # Example allergen property: P2674 (has quality)
        query = f"""
        SELECT ?allergenLabel WHERE {{
          wd:{qid} wdt:P2674 ?allergen .
          SERVICE wikibase:label {{ bd:serviceParam wikibase:language "en". }}
        ...
        allergens = [binding["allergenLabel"]["value"] for binding in results["results"]["bindings"]]
        return Response({"allergens": allergens})

    @swagger_auto_schema(
        operation_description="Get nutrition info of an ingredient.",
        manual_parameters=[
            openapi.Parameter('name', openapi.IN_QUERY, description="Ingredient name", type=openapi.TYPE_STRING, required=True)
        ],
        tags=["IngredientWikidata"]
    )
    @action(detail=False, methods=['get'], url_path='nutrition')
    def nutrition(self, request):
        ...
        """
        results = self.run_sparql_query(query)
        nutrition = {binding["propertyLabel"]["value"]: binding["value"]["value"] for binding in results["results"]["bindings"]}
        return Response(nutrition)

    @swagger_auto_schema(
        operation_description="Check if ingredient/meal is vegan.",
        manual_parameters=[
            openapi.Parameter('name', openapi.IN_QUERY, description="Ingredient or meal name", type=openapi.TYPE_STRING, required=True)
        ],
        tags=["IngredientWikidata"]
    )
    @action(detail=False, methods=['get'], url_path='is-vegan')
    def is_vegan(self, request):
        name = request.query_params.get("name")
        if not name:
            return Response({'error': 'Query parameter "name" is required.'}, status=status.HTTP_400_BAD_REQUEST)
        ...
        # Heuristic: see if it's an instance/subclass of Q25340 (vegan food)
        query = f"""
        ASK {{
          wd:{qid} wdt:P279* wd:Q25340 .
        }}
        ...
        return Response({"is_vegan": result.get("boolean", False)})
    ...
    @swagger_auto_schema(
        operation_description="Get category of an ingredient.",
        manual_parameters=[
            openapi.Parameter('name', openapi.IN_QUERY, description="Ingredient name", type=openapi.TYPE_STRING, required=True)
        ],
        tags=["IngredientWikidata"]
    ...
    def category(self, request):
        ...
        """
        results = self.run_sparql_query(query)
        categories = [binding["categoryLabel"]["value"] for binding in results["results"]["bindings"]]
        return Response({"category": categories[0] if categories else None})
    ...
    @swagger_auto_schema(
        ...
        manual_parameters=[
            openapi.Parameter(
                'name',
                openapi.IN_QUERY,
                description="Ingredient or meal name",
                type=openapi.TYPE_STRING,
                required=True
            )
        ],
