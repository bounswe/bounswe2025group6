...
class ShoppingList {
  ...
  final int portionCount; // Number of portions for the shopping list
  ...
  ShoppingList({
    ...
    this.portionCount = 1, // Default to 1 portion
  });
  ...
  /// [portionCount] multiplies all ingredient quantities and costs
  static ShoppingList fromRecipes(List<Recipe> recipes, {int portionCount = 1}) {
    ...
    for (var recipe in recipes) {
      for (var ingredient in recipe.ingredients) {
        ...
        // Multiply ingredient quantity by portion count
        final adjustedQuantity = ingredient.quantity * portionCount;
        
        if (aggregatedItems.containsKey(key)) {
          // Add to existing quantity
          final existingItem = aggregatedItems[key]!;
          aggregatedItems[key] = existingItem.copyWith(
            quantity: existingItem.quantity + adjustedQuantity,
    ...
    // Calculate total cost (sum of all recipe costs) multiplied by portion count
    double totalCost = 0.0;
    for (var recipe in recipes) {
      if (recipe.costPerServing != null) {
        totalCost += recipe.costPerServing! * portionCount;
      }
    }
  ...
  String toPlainText({
    ...
    // Show portion count
    buffer.writeln('👥 $portionsLabel: $portionCount');
    ...
    for (var item in items) {
      ...
      buffer.writeln('$checkMark ${item.quantity.toStringAsFixed(2)} ${item.unit} ${item.ingredientName}');
    }
  ...
  factory ShoppingList.fromJson(Map<String, dynamic> json) {
    return ShoppingList(
      ...
      portionCount: json['portionCount'] as int? ?? 1,
    );
  }
