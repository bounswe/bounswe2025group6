...
class ShoppingList {
  ...
  /// Convert to shareable plain text format
  String toPlainText({String currencySymbol = '\$'}) {
    final buffer = StringBuffer();
    buffer.writeln('🛒 Shopping List');
    buffer.writeln('Generated: ${createdAt.toString().split('.')[0]}');
    buffer.writeln('');
    ...
    buffer.writeln('');
    buffer.writeln('🥕 Ingredients (${items.length} items):');
    buffer.writeln('');
    ...
    buffer.writeln('');
    buffer.writeln('💰 Total Cost: $currencySymbol${totalCost.toStringAsFixed(2)}');
    
    if (costByRetailer != null && costByRetailer!.isNotEmpty) {
      buffer.writeln('');
      buffer.writeln('🏪 Cost by Retailer:');
      ...
      if (cheapest != null) {
        buffer.writeln('  ⭐ Best: $cheapest');
      }
    }
    
    buffer.writeln('');
    buffer.writeln('Generated by FitHub Meal Planner');
